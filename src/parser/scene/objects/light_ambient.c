...
/*   Updated: 2023/09/11 09:58:51 by htsang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_mrt_scene_parser_atom	mrt_scene_parser_light_ambient_parameters(\
...
t_mrt_scene_parser_atom input, union u_ft_tobject option)
{
	...
	result = mrt_scene_parser_identifier(ft_parser_atom_payload_set(\
		input, ft_tobject_ptr(&is_unique)), ft_tobject_str("A"));
	if (!result.is_valid)
	...
	result = mrt_scene_parser_light_ambient_parameters(\
		ft_parser_atom_payload_set(\

...
/*   Updated: 2023/09/08 15:49:32 by htsang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_mrt_scene_parser_atom	mrt_scene_parser_plane_parameters(\
t_mrt_scene_parser_atom input, union u_ft_tobject delimiter)
{
	...
	return (mrt_combinator_struct_fields((struct s_ft_parser_entity[3]){\
		...
		ft_parser_entity(&mrt_scene_parser_vec3, ft_tobject_ptr(range)), \
...
t_mrt_scene_parser_atom input, union u_ft_tobject option)
{
	...
	result = mrt_scene_parser_identifier(ft_parser_atom_payload_set(\
		input, ft_tobject_ptr(&is_unique)), ft_tobject_str("pl"));
	if (!result.is_valid)
	...
	result = mrt_scene_parser_plane_parameters(ft_parser_atom_payload_set(\
